...
SpriteMorph.prototype.initBlocks = function () {
    ...
    */
        ...
        doParallelForEach: {
            type: 'command',
            category: 'control',
            spec: 'for each %upvar of %l %parallel %cs',
            defaults: [localize('item')]
        },
        reportExecutionTime: {
        	type: 'reporter',
        	category: 'control',
        	spec: 'report execution time %cmdRing'
        },
    /*
        doRunWithInputList: {
            type: 'command',
    ...
    */
        ...
        reportParallelAdd: {
        	type: 'reporter',
        	category: 'lists',
        	spec: 'parallelAdd %l in %n'
        },
        */
        reportParallelMap: {
        	type: 'reporter',
        	category: 'lists',
        	spec: 'parallelMap %repRing over %l %parallel'
        },
        reportMapReduce: {
        	type: 'reporter',
        	category: 'lists',
//        	spec: 'map %repRing reduce %repRing over %s'
        	spec: 'map %repRing reduce %repRing over %l %parallel'
...
SpriteMorph.prototype.blockTemplates = function (category) {
    ...
    */
    ...
    ///////////////////////////////
        ...
        //blocks.push(block('reportParallelAdd'));
...
SpriteMorph.prototype.createPClone = function(block,cname) {
	// create clone here with only this process block - no hat block.
	var new_sprite,
		stage = this.parentThatIsA(StageMorph);
	
	//this.scripts.add(block);
	new_sprite = this.fullCopy();
	new_sprite.scripts.add(block);
	new_sprite.parts.forEach(function (part) {
		part.clonify(stage);
	});
	stage.cloneCount += 1;
	new_sprite.cloneOriginName = new_sprite.isClone ? 
			new_sprite.cloneOriginName : new_sprite.name;
	new_sprite.isClone = true;
//	new_sprite.name = '';
	new_sprite.name = cname;
	stage.add(new_sprite);
