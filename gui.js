...
IDE_Morph.prototype.createControlBar = function () {
    ...
    function makeToggleButton(action, labelArray, query)
    {
    ...
    function makePushButton(action, label)
    {
    ...
    //stageSizeButton
    stageSizeButton = makeToggleButton
    (
    	'toggleStageSize',
    	[
    	...
    	function () { return myself.isSmallStage; }
    );
    // stageSizeButton.hint = 'stage size\nsmall & normal';
    stageSizeButton.drawNew();
    stageSizeButton.fixLayout();
    stageSizeButton.refresh();
    this.controlBar.add(stageSizeButton);
    this.controlBar.stageSizeButton = stageSizeButton; // for refreshing

    //appModeButton
    appModeButton = makeToggleButton
    (
        'toggleAppMode',
        [
        ...
        function () { return myself.isAppMode; }
    );
    // appModeButton.hint = 'app & edit\nmodes';
    appModeButton.drawNew();
    appModeButton.fixLayout();
    appModeButton.refresh();
    this.controlBar.add(appModeButton);
    this.controlBar.appModeButton = appModeButton; // for refreshing

    // stopButton
    stopButton = makePushButton
    (
        'stopAllScripts',
    ...
    (
        ...
        function () { return myself.isPaused(); }
    );
    pauseButton.labelColor = new Color(255, 220, 0);
    // pauseButton.hint = 'pause/resume\nall scripts';
    pauseButton.drawNew();
    ...
    // projectButton
    this.controlBar.projectButton = makePushButton
    (
        'projectMenu',
        new SymbolMorph('file', 14)
        //'\u270E'
    ); // for menu positioning, add to control bar
    this.controlBar.add(this.controlBar.projectButton);
    // this.controlBar.projectButton.hint = 'open, save, & annotate project';
    this.controlBar.projectButton.drawNew();
    this.controlBar.projectButton.fixLayout();

    // settingsButton
    settingsButton = makePushButton
    (
        'settingsMenu',
        new SymbolMorph('gears', 14)
        //'\u2699'
    );
    // settingsButton.hint = 'edit settings';
    settingsButton.drawNew();
    settingsButton.fixLayout();
    this.controlBar.add(settingsButton);
    this.controlBar.settingsButton = settingsButton; // for menu positioning

    // cloudButton
    cloudButton = makePushButton
    (
        'cloudMenu',
        new SymbolMorph('cloud', 11)
    );
    // cloudButton.hint = 'cloud operations';
    cloudButton.drawNew();
    cloudButton.fixLayout();
    ...
    this.controlBar.fixLayout = function () {
        ...
        myself.controlBar.projectButton.setCenter(myself.controlBar.center());
        myself.controlBar.projectButton.setRight(cloudButton.left() - padding);
...
IDE_Morph.prototype.createSpriteEditor = function () {
    ...
    if (this.currentTab === 'scripts') {
    	...
    	this.spriteEditor = new DatastoreMorph(
    		this.currentSprite,
    		this.sliderColor
    	);
    	this.spriteEditor.color = this.groupColor;
...
IDE_Morph.prototype.settingsMenu = function () {
    ...
    function addPreference(label, toggle, test, onHint, offHint, hide) {
        var on = '\u2611 ',   // ballot box with check - jeez can this code be any more cryptic?!
            off = '\u2610 ';  // ballot box without check
